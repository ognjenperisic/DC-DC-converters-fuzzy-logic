...
clear all
clc
cd c:\MOJI_RADOVI\ETF_DIPLOMA\FUZZY\
...
A=[-(r*rl+r*rc+rl*rc)/((r+rc)*l) -r/((r+rc)*l);  r/((rc+r)*c)  -1/((r+rc)*c)]; 
...
pr_greska=0;
niz_g = zeros(1, broj);
niz_p_g = zeros(1, broj);
%fuzzy_i = zeros(1, broj);
dk=0;
...
for i=1:broj                    % pocetak petlje
     ...
     if (rem(i, 1000)==0) 
        slika(y(1:i), izlaz(1:i), niz_g(1:i), niz_p_g(1:i))            % procedura za prikazivanje rezultata
        pause(3)
     end  
...
s=s/(broj)
close all
%slika2(y,izlaz,niz_g,niz_p_g,broj)   % procedura za prikazivanje rezultata
